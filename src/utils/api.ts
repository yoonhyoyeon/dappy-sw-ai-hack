...
export async function getEscapePathAndAnalysis(currentLocation: string) {
  const TEST_AI_SERVER_URL = 'http://43.202.81.16:3000';
  
  try {
    console.log('=== 구역 상태 및 대피 경로 API 호출 ===');
    console.log('요청 URL:', `${TEST_AI_SERVER_URL}/get-escape-path`);
    console.log('요청 Body:', { current_location: currentLocation });
    
    const response = await fetch(`${TEST_AI_SERVER_URL}/get-escape-path`, {
      method: 'POST',
      mode: 'cors',
      headers: {
